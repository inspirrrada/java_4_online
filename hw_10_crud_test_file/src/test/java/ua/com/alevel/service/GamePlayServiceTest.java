...
public class GamePlayServiceTest {
    ...
    private static String PLAYERSFILE = "playersTest.json";
    private static String GAMESFILE = "gamesTest.json";
    private static GamePlayService GAME_PLAY_SERVICE_IMPL = new GamePlayServiceImpl(PLAYERSFILE, GAMESFILE);
    ...
    @BeforeAll
    public static void createStorageFiles() {
        File playersFile = new File(PLAYERSFILE);
        File gamesFile = new File(GAMESFILE);
        try {
            playersFile.createNewFile();
            FileWriter writer = new FileWriter(playersFile);
            writer.write(new ArrayList().toString());
            writer.flush();

            gamesFile.createNewFile();
            writer = new FileWriter(gamesFile);
            writer.write(new ArrayList().toString());
            writer.flush();
        } catch (IOException e) {
    ...
    public void checkUpdatePlayerEmailWithNewCorrectValue() {
        ...
        Assertions.assertEquals(newEmail, GAME_PLAY_SERVICE_IMPL.getPlayerByIdOrNull(playerId).getEmail());
    ...
    public void checkUpdatePlayerNicknameWithNewCorrectValue() {
        ...
        Assertions.assertEquals(newNickname, GAME_PLAY_SERVICE_IMPL.getPlayerByIdOrNull(playerId).getNickname());
    ...
    public void checkUpdateGameNameWithNewCorrectValue() {
        ...
        Assertions.assertEquals(newGameName, GAME_PLAY_SERVICE_IMPL.getGameByIdOrNull(gameId).getName());
    }
    ...
    public void checkUpdateGameTypeWithNewCorrectValue() {
        ...
        Assertions.assertEquals(newIsCommandGame, GAME_PLAY_SERVICE_IMPL.getGameByIdOrNull(gameId).isCommandGame());
    }
    ...
    public void checkAttachGameToThePlayer() {
        ...
        GAME_PLAY_SERVICE_IMPL.addGameToPlayerInAllDb(game.getId(), player.getId());
        //get info from file
        player = GAME_PLAY_SERVICE_IMPL.getPlayerByIdOrNull(player.getId());
        game = GAME_PLAY_SERVICE_IMPL.getGameByIdOrNull(game.getId());
    ...
    public void deleteTestFiles() {
        File playersFile = new File("playersTest.json");
        File gamesFile = new File("gamesTest.json");
        playersFile.delete();
        gamesFile.delete();
    }
}
